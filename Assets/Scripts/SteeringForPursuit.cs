...
public class SteeringForPursuit : Steering {
    ...
    public override Vector3 Force()
    {
        ...
        //如果夹角大于0，切追逐者基本面对这逃避者，那么直接向逃避者当前位置移动
        if ((Vector3.Dot(toTarget, transform.forward) > 0) && (relativeDirection < -0.95f)) {
            desiredVelocity = (target.transform.position - transform.position).normalized * maxSpeed;
            return (desiredVelocity - m_vehicle.velocity);
        }
        //计算预测时间，正比于追逐着与逃避者的距离，反比于追逐者和逃避者的速度和；
        float lookheadTime = toTarget.magnitude / (maxSpeed + target.GetComponent<Vehicle>().velocity.magnitude);
        //计算预期速度；
